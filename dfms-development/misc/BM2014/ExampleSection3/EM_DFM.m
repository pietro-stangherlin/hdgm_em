function Res = EM_DFM(X,P)

% function to estimate the parameters of the dynamic factor model for 
...
r = P.r; %no of factors
p = P.p; %no of lags in factor VAR
max_iter = P.max_iter; %maximal no of iterations in the EM
...
optNaN.method = 6; % Remove leading and closing nans, replace missing values with random draws
% optNaN.method = 4; % Remove leading and closing nans, replace missing values with splines
% optNaN.method = 5; % Replace missing values with splines
optNaN.k = 3;

[A, C, Q, R, Z_0, V_0] = InitCond(xNaN,r,p,optNaN);

%% 2-step estimators
Zsmooth = runKF(xNaN', A, C, Q, R, Z_0, V_0)';

% smoothed observations
x_sm = Zsmooth(2:end,:)*C';
% smoothed observations with original mean and std
Res.X_sm2st = repmat(Wx,T,1).*x_sm+repmat(Mx,T,1);
% factors
Res.F2st = Zsmooth(2:end,1:r);
y = X;
y(isnan(y)) = Res.X_sm2st(isnan(y));
Res.y2st = y;
...
initV = reshape((eye(rp2)-kron(A,A))\Q(:),r*p,r*p);
