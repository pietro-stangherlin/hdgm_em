...
# options = list(
#   method = 2L,
#   k = 3L        # order of the moving average (lags and leads) for replacing the missing observations
# )
...
# Translations of original code
         
# case 1  % replace all the missing values         
# remNaN1 <- function(X, k) {
#   d <- dim(X)
#   T <- d[1L]
#   N <- d[2L]
#   k2 = 2L * k + 1L
...
#   for (i in 1:N) {
#     x = X[, i]
#     x[indNaN[, i]] = fmedian.default(x)
#     x_MA = filter(c(rep(x[1L], k), x, rep(x[T], k)), rep(1, k2)/k2, sides = 1L);
#     x[indNaN[ ,i]] = x_MA[-(1:(k2-1L))][indNaN[, i]];
#     X[, i] = x;
#   }
#   return(list(X = X, indNaN = indNaN))
# }
...
# Not faster...
# remNaN2o <- function(X, k, threshold = 0.8) {
#   d <- dim(X)
#   T <- d[1L]
#   N <- d[2L]
#   k2 = 2L * k + 1L
...
#   if(rem1[1L] || rem1[T]) {
#     rem1g = groupid(rem1)
#     nanLE = c(if(rem1[1L]) which(rem1g == 1L) else NULL, 
#               if(rem1[T]) which(rem1g == rem1g[T]) else NULL)
#     X = X[-nanLE, ] 
#     indNaN = indNaN[-nanLE, ]
#   }
