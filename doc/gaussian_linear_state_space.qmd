...
where $H = S_{11} - S_{10} \Phi^{\top} - \Phi S_{10}^\top + \Phi S_{00} \Phi^\top$ and $H^{(1)}$ is the upper matrix, while $H^{(2)}$ the lower matrix.

The optimization of components with $Q^{(1)}$ is the same as before using Brent method while the diagonal structure of $Q^{(2)}$ and its inverse permits analytical updates.
