...
Prior independence is assumed among the parameters. Here $1/\sigma^2_Y$ has a $\Gamma(a_{\sigma^2_Y},b_{\sigma^2_Y})$ prior; to force stationarity $g \sim Unif(-1,1)$; in order to ensure identifiability it has to be $\alpha \geq 0$ this can be accomplished by a reparameterization or a positive prior such as a$\Gamma(a_{\alpha},b_{\alpha})$. The $\theta > 0$ spatial distance parameter can also have a $\Gamma(a_{\theta},b_{\theta})$ prior and finally $u \sim N_p(u_0,\sigma^2_{u_0} I_p)$.
...
### Remove the parametric spatial covariance assumption

### Add time varying covariates instead of fixed effetcs

### Go over the linear Gaussian distribution assumption

Use models such the extended Kalman Filter [@DurbinKoopman] or consider the quantile dynamic models.
