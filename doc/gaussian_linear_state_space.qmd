...
$$
Q_{ij} = \exp (-|| y_{t,i} - y_{t,j} ||_2 / \theta)
$$ {#eq-exp-cor-el}

with scalars $\alpha$, $|g|<1$ (implied stationary state autoregression structure), $\sigma^2_{Y}, \theta > 0$.
...
The $\sigma^2_Y$ update is $(\sigma^2_Y)^j = \frac{1}{qT} \text{tr} \left( \sum^{T}_{t=1} \Omega_t \right)$ where $\Omega_t = y_t - \Gamma_t u - Ax_t + \alpha^2 P^T_t$.

Since the scaling is given by $\alpha$, it can't be assumed known (unless in this parameterization) its update is $\alpha^j = \frac{\sum^T_{t=1}\text{tr}\left(\psi_t x^\top_t \right)}{\text{tr} \left( S_{11} \right)}$
...
Since this is a univariat optimization problem Brent method is used, the $\theta$ dimension increases other methods should be employed such Nelder-Mead.
