...
// [[Rcpp::export]]
Rcpp::List EMHDGM_statescale(const arma::mat& y, // observation matrix (n x T) where T = n. obs
                  const arma::mat& dist_matrix, // state distance matrix (complete data)
                  double alpha0, // initial observation matrix scaling
                  const arma::vec beta0, // initial fixed effect
                  double theta0, // initial state covariance parameter (exponential)
  ...
  if(bool_mat == true){
    res = EMHDGM_state_scale_cpp_mat(inp);
  }else{
    res = EMHDGM_state_scale_cpp(inp);
  }
...
// [[Rcpp::export]]
Rcpp::List EMHDGM_diag(const arma::mat& y, // observation matrix (n x T) where T = n. obs
                  const arma::mat& dist_matrix, // state distance matrix (complete data)
                  double alpha0, // initial observation matrix scaling
                  const arma::vec beta0, // initial fixed effect
                  double theta0, // initial state covariance parameter (exponential)
  ...
  if(bool_mat == true){
    res = EMHDGM_diag_cpp_mat(inp);
  }else{
    res = EMHDGM_diag_cpp(inp);
  }
