...
EMOutput EMHDGM_diag_cpp_core(EMInput& em_in) {
  ...
  double alpha_temp = em_in.alpha0;
  double theta_temp = em_in.theta0;
  //keep g0 scalar so we don't have to change the em.in structure
  arma::vec g_vec_temp = arma::ones(q) * em_in.g0;
  double sigma2_temp = em_in.sigma20;
  ...
  // NOTE: parameter dimension has to be changed if the parameters space change
  // also, zero is not a perfect initialization value
  // q parameters for Phi diagonal (g param) + 3 for alpha, theta, sigma^2
  arma::mat par_history = arma::mat(q + 3, em_in.max_iter + 1);
  arma::mat beta_history = arma::mat(p, em_in.max_iter + 1);

  arma::vec full_vec(q + 3);

  full_vec(0) = alpha_temp;
  full_vec.subvec(1, q) = g_vec_temp;   // q elements
  full_vec(q + 1) = theta_temp;
  full_vec(q + 2) = sigma2_temp;

  par_history.col(0) = full_vec;
  beta_history.col(0) = beta_temp;
  ...
  for (int iter = 1; iter < em_in.max_iter + 1; ++iter) {
    ...
    // update state space matrices
    A_temp = alpha_temp * Iqq;
    Phi_temp = arma::diagmat(g_vec_temp);
    Q_temp = ExpCor(em_in.dist_matrix, theta_temp);
    R_temp = sigma2_temp * Iqq;
    ...
    // g update
    g_vec_temp = arma::diagvec(S10 * arma::inv_sympd(S00));

    // Update parameters history
    full_vec(0) = alpha_temp;
    full_vec.subvec(1, q) = g_vec_temp;
    full_vec(q + 1) = theta_temp;
    full_vec(q + 2) = sigma2_temp;


    par_history.col(iter) = full_vec;
    beta_history.col(iter) = beta_temp;
  }
